...
PY_FIT = REPORTS / "py_nb_benchmark.json"
TRUE_PARAMS = DATA / "pcount_nb_true_params.json"


def build_designs() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    counts = pd.read_csv(DATA / "pcount_nb_counts.csv")
    covs = pd.read_csv(DATA / "pcount_nb_site_covs.csv")
...
def load_theta() -> tuple[np.ndarray, str]:
    ...
    theta = np.asarray(
        truth["beta"] + truth["detection_logits"] + [truth["log_r"]], dtype=np.float64
    )
    return theta, "true"
...
def main() -> None:
    ...
    problem = _core.PCountNegBinProblem(y, X, W, 100)
    repetitions = 1000
    warmups = 20
    ...
    (REPORTS / "py_nb_loglik_benchmark.json").write_text(json.dumps(payload, indent=2) + "\n")
