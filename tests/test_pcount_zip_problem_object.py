...
def test_zip_problem_object_matches_raw_function():
    y = np.array([[0.0, 0.0], [2.0, np.nan]], dtype=np.float64)
    ...
    problem = _core.PCountZIPProblem(y, X, W, 9)
    assert problem.n_sites == 2
    assert problem.n_visits == 2
    assert problem.K == 9
    raw = _core.pcount_zip_loglik(y, X, W, theta, 9)
