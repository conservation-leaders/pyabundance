...
write_not_run <- function(message) {
  ...
  writeLines(payload, file.path(reports, "r_nb_benchmark.json"))
}
...
result <- tryCatch({
  library(unmarked)
  counts <- read.csv(file.path(data_dir, "pcount_nb_counts.csv"))
  covs <- read.csv(file.path(data_dir, "pcount_nb_site_covs.csv"))
