...
PY_JSON = REPORTS / "py_zip_benchmark.json"
PY_LOGLIK_JSON = REPORTS / "py_zip_loglik_benchmark.json"
R_JSON = REPORTS / "r_zip_benchmark.json"
OUT = REPORTS / "BENCHMARK_RESULTS_ZIP.md"
...
def coefficient_comparison(py: dict[str, Any] | None, r: dict[str, Any] | None) -> str:
    ...
    return "not forced; raw Python/R vectors recorded because ZIP psi naming may differ"


def psi_comparison(py: dict[str, Any] | None, r: dict[str, Any] | None) -> str:
    if not py or not r or r.get("status") != "completed":
        return "not assessed"
    return (
        f"Python logit_psi={fmt(py.get('logit_psi'))}, psi={fmt(py.get('psi'))}; "
        "R zero-inflation parameterization not assumed from coefficient order"
    )
...
def main() -> None:
    ...
    notes = [
        ...
        "Low detection, low abundance, and structural zeros can all produce many observed "
        "zero counts.",
    ...
    if py and py.get("status") == "completed":
        ...
        notes.append(f"Python ZIP benchmark did not complete: {py.get('message', 'no message')}")
    else:
        notes.append("Python ZIP benchmark was not run.")
    ...
    if py_ll and py_ll.get("status") == "completed":
        ...
        notes.append("Python ZIP direct likelihood benchmark was not run or did not complete.")
    ...
    if r and r.get("status") == "completed":
        ...
        notes.append(f"R ZIP benchmark was not run: {r.get('message', 'no message')}")
        r_env = r.get("R_version", r_env)
    else:
        notes.append("R ZIP benchmark was not run; reports/r_zip_benchmark.json is missing.")

    text = f"""# Zero-Inflated Poisson Benchmark Results
...
- psi comparison: {psi_comparison(py, r)}
...
- direct ZIP likelihood median: {fmt(direct_ms)} ms
