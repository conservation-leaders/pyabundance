...
json_str_vec <- function(v) paste0("[\"", paste(json_escape(v), collapse = "\", \""), "\"]")
...
write_not_run <- function(message) {
  ...
  writeLines(payload, file.path(reports, "r_zip_benchmark.json"))
}
...
result <- tryCatch({
  library(unmarked)
  counts <- read.csv(file.path(data_dir, "pcount_zip_counts.csv"))
  covs <- read.csv(file.path(data_dir, "pcount_zip_site_covs.csv"))
  ...
  payload <- paste0(
    ...
    '  "coefficient_names": ', json_str_vec(names(coefs)), ',\n',
    '  "R_version": "', json_escape(R.version.string), '",\n',
    '  "unmarked_version": "', as.character(utils::packageVersion("unmarked")), '",\n',
    '  "platform": "', json_escape(R.version$platform), '",\n',
    '  "dataset": {"n_sites": ', nrow(y), ', "n_visits": ', ncol(y), ', "K": 100, "repetitions": 3}\n',
    '}\n'
  )
  writeLines(payload, file.path(reports, "r_zip_benchmark.json"))
  TRUE
