...
PY_FIT = REPORTS / "py_benchmark.json"
TRUE_PARAMS = DATA / "pcount_true_params.json"
...
def load_theta() -> tuple[np.ndarray, str]:
    ...
    theta = np.asarray(truth["beta"] + truth["detection_logits"], dtype=np.float64)
    return theta, "true"
...
def main() -> None:
    ...
    problem = _core.PCountPoissonProblem(y, X, W, 60)
    repetitions = 1000
    warmups = 20
    ...
    (REPORTS / "py_loglik_benchmark.json").write_text(json.dumps(payload, indent=2) + "\n")
