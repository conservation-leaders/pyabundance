...
write_not_run <- function(message) {
  ...
  writeLines(payload, file.path(reports, "r_se_comparison.json"))
}
...
result <- tryCatch({
  ...
  fit <- pcount(~ visit - 1 ~ x, data = umf, K = 60, starts = rep(0, 5), mixture = "P", se = TRUE, engine = "C", threads = 1, method = "BFGS")
  coefs <- coef(fit)
  ses <- tryCatch({ as.numeric(SE(fit)) }, error = function(e) rep(NA_real_, length(coefs)))
  payload <- paste0(
    '{\n',
    '  "status": "completed",\n',
    '  "logLik": ', format(as.numeric(logLik(fit)), digits = 17), ',\n',
    '  "coefficients": ', json_vec(as.numeric(coefs)), ',\n',
    '  "standard_errors": ', json_vec(ses), ',\n',
