use ecoabundance_core::pcount_poisson::{
    ...
    PCountPoissonProblem as CorePCountPoissonProblem,
};
...
#[pyclass(name = "PCountPoissonProblem", module = "pyabundance._core")]
pub struct PyPCountPoissonProblem {
    problem: CorePCountPoissonProblem,
}

#[pymethods]
impl PyPCountPoissonProblem {
    #[new]
    pub fn new(
        ...
        let problem =
            CorePCountPoissonProblem::new(y_vec, x_vec, w_vec, k, dims).map_err(value_error)?;
        Ok(Self { problem })
    }
    ...
    pub fn predict_lambda(
        ...
        py.allow_threads(|| self.problem.predict_lambda(&beta_vec))
            .map_err(value_error)
    }
