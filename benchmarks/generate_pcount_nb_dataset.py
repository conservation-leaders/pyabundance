...
def main() -> None:
    ...
    gamma_rate = rng.gamma(shape=r, scale=lam / r)
    latent_n = rng.poisson(gamma_rate)
    ...
    pd.DataFrame(y, columns=["y1", "y2", "y3"]).to_csv(
        DATA_DIR / "pcount_nb_counts.csv", index=False
    )
    pd.DataFrame({"x": x}).to_csv(DATA_DIR / "pcount_nb_site_covs.csv", index=False)
    (DATA_DIR / "pcount_nb_true_params.json").write_text(
        json.dumps(
            {
                "seed": seed,
                "n_sites": n_sites,
