use ecoabundance_core::pcount_negbin::{
    log_negative_binomial_pmf_mean_size as core_log_negbin_pmf,
    pcount_negbin_loglik as core_loglik, PCountNegBinProblem as CorePCountNegBinProblem,
};
...
#[pyclass(name = "PCountNegBinProblem", module = "pyabundance._core")]
pub struct PyPCountNegBinProblem {
    problem: CorePCountNegBinProblem,
}

#[pymethods]
impl PyPCountNegBinProblem {
    #[new]
    pub fn new(
        ...
        let problem =
            CorePCountNegBinProblem::new(y_vec, x_vec, w_vec, k, dims).map_err(value_error)?;
        Ok(Self { problem })
    }
...
pub fn pcount_negbin_loglik(
    ...
    if theta_view.len() != expected_theta {
        return Err(PyValueError::new_err(format!(
            "theta length {} must equal X columns + W detection columns + log_r {expected_theta}",
            theta_view.len()
        )));
    }
...
#[pyfunction]
pub fn log_negative_binomial_pmf_mean_size(n: u64, mean: f64, size: f64) -> PyResult<f64> {
    core_log_negbin_pmf(n, mean, size).map_err(value_error)
}
